...
export async function getConnectedWorktrees(repoPath) {
    try {
...
export async function createLocalSchema(repoPath, schemaName, artifacts) {
    ...
    await execPromise(`openspec schema init "${schemaName}" --artifacts "${artifactsList}" --no-default`, resolvedRepoPath);
}
...
export async function getChangeWorktree(repoPath, changeName) {
    try {
        const output = await execPromise('git worktree list --porcelain', repoPath);
...
export async function getChangeMetadata(repoPath, changeName) {
    ...
    if (fs.existsSync(changeConfigFile)) {
        ...
        if (data.proposeEngine)
            proposeEngine = data.proposeEngine;
    }
...
export function getChangeFilesContent(repoPath, changeName) {
    ...
    let result = '';
    function readDirectory(dir, relativeDir = '') {

...
- **Key Changes:**
  ...
  - 12 new/changed vitest tests (C9: reuse-lowest-free, no emit for live id, server-list-after-reload; C16: no Attach button, iTerm2 always zsh, no send-message routing, clear no-op), red-verified against the pass-1 implementation (C9 reload/collision reds) and the pass-1 explicit-state code (removal reds); the two TerminalPane removal guards are green-only by nature (asserting absence).

...
- **Key Changes:**
  ...
  - Server: natural PTY exit now broadcasts `terminal-sessions-updated` (explicit closes already did).
  - Client (`TerminalPane`): new `terminal-exit` and `terminal-error` handlers drop the dead/rejected tab and switch to the first remaining session if the dead one was active. `handleSwitchSession`'s early-return guard now reads `activeSessionRef` because mount-time socket handlers capture a stale `activeSession` state closure.
  - 10 new server vitest tests (cap x2, reap + cancel + main-exempt via fake timers, spawn-throw guard, single-emit, input guard, id uniqueness, exit broadcast) and 3 new client tests (exit removes active tab + switches, background exit keeps active, error reverts optimistic tab); the ptyService suite now kills every PTY it spawns in afterEach (previously leaked real shells across the run) and asserts no stray pids survive.

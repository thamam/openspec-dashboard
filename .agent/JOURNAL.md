...
### [Cycle #016] - 2026-08-31 - Terminal Session Id Collision & Agent-Session State (C9, C16)
- **Human Pain Point / Friction:** Creating a terminal tab after closing one could hijack an existing session id (create emit + switch fired for an id that already existed); the "Attach agent" affordance and prompt→agent routing were governed by a log-parsing heuristic that had silently never worked (every /api/execute reply ends with an exit marker), so the UI presented a capability that was effectively dead — an exception-based-review violation.
- **Matched UX Idea:** Never present affordances without a backing capability; state the user can see should be state the app actually holds.
- **Key Changes:**
  - `TerminalPane/index.tsx` `handleCreateSession` now uses a monotonic ref counter (`session-N` never reused after closes) instead of `sessions.length + 1`; the collision guard is gone because a collision is no longer possible.
  - C16 disposition: the detection result had live consumers (TerminalPane's Attach/iTerm buttons, App's /api/send-message routing), so removal was rejected. The active agent session is now explicit App state — set on `tmux attach -t <agent-N>`, cleared by `exit`/`disconnect`, passed to TerminalPane as the `activeAgentSession` prop.
  - With detection gone, App's `terminalLines` array became write-only (it only fed detection since the PTY rework; xterm owns the visible scrollback), so the whole pipeline was removed: the log state, `captureTmuxPane`, `cleanAnsiText`, and the now-orphaned `client/src/terminal/` modules (`sessionDetect.ts`, `lineBuffer.ts`) with their tests. `executeCommand` still POSTs /api/execute, drains the stream, and refreshes artifacts — Command Center behavior unchanged.
  - 7 new vitest tests (C9 collision + no-spurious-emit, C16 prop-driven attach button + no log derivation, App-level attach→route / exit→detach / non-agent-no-route), all red-verified pre-fix except the non-agent regression guard (documented in PR).
